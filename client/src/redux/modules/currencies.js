...
export const fetchCurrenciesEpic = action$ => action$.pipe(
	...
	mergeMap(action => ajax.get(`${url}get_currencies`).pipe(
...
export const fetchNewCurrenciesEpic = action$ => action$.pipe(
	...
	mergeMap(action => ajax.post(`${url}get_new_currencies`, action.payload, { 'Content-Type': 'application/json' }).pipe(
...
export const changeInputValueCurrencyEpic = action$ => action$.pipe(
	...
	mergeMap(action => concat(
		...
		from(ajax.post(`${url}cur_rate`, action.payload, { 'Content-Type': 'application/json' })).pipe(
...
export const addNewCurrencyFulfilledEpic = action$ => action$.pipe(
  ...
  mergeMap(action => ajax.post(`${url}add_currency`, action.payload, { 'Content-Type': 'application/json' }).pipe(
